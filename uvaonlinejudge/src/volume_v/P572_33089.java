...
class P572_33089 {
    ...
    private void begin() {
        String input = P572_33089.readLn();
        ...
        while (m != 0 || n != 0) {
            ...
            for (int i = 0; i < m; i++) {
                mapa[i] = new char[n];
                String linha = P572_33089.readLn();
                for (int j = 0; j < n; j++) {
                    mapa[i][j] = linha.charAt(j);
                }
            }
            System.out.println(this.conta(mapa, m, n));
            input = P572_33089.readLn();
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (mapa[i][j] == '@' && numMap[i][j] == 0) {
                    max++;
                    this.floodFill(mapa, numMap, i, j, m, n, max);
                }
            }
        }
