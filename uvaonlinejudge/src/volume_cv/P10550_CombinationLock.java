...
/**
 * UVa Online Judge Problem #10550 - Combination Lock
 * 
...
public class P10550_CombinationLock {
  ...
  private static int MODULO = 40;

  private static int delta(int from, int to, int direction) {
    int delta = (MODULO + direction*to - direction*from)%MODULO;
    // System.out.println("(" + direction + ")" + from + " to " + to + " = " + delta);
    return delta;
  }
  ...
  public static void main(String [] args) {
    ...
    while (p != 0 || a !=0 || b != 0 || c != 0) {
      // Two full clockwise + 1 full counter_clockwise
      int steps = 3 * MODULO;

      // Delta to a
      steps += delta(p, a, CLOCKWISE);
      steps += delta(a, b, COUNTER_CLOCKWISE);
      steps += delta(b, c, CLOCKWISE);
